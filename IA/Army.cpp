...
Unit& Army::getNearestUnit(const Point& p)
{
    ...
    Unit* result = nullptr;
    double minDist = DBL_MAX;// std::numeric_limits<double>::max();
    for(auto it = units_.begin(); it != units_.end(); ++it) {
        double d = (*it)->getPosition().distance(p);
        if(d < minDist) {
            minDist = d;
            result = it->get();
        }
...
{
    std::for_each(units_.begin(), units_.end(), [this](const std::shared_ptr<Unit>& unit) {
